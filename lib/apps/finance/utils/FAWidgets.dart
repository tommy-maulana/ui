...
Widget faAppButtonWidget(BuildContext context, {String? title, Function? onTap, bool isHome = false}) {
  return AppButton(
    width: context.width(),
    color: isHome ? Colors.grey[300] : fa_color_secondary,
    child: Text(title.validate(), style: boldTextStyle(color: isHome ? Colors.grey : white)),
    shapeBorder: RoundedRectangleBorder(borderRadius: BorderRadius.all(Radius.circular(30))),
    onTap: onTap,
  );
}

Widget phoneWithCountryPickerAppTextFiled({required TextEditingController controller, focus, TextFieldType? textFieldType}) {
  return Container(
    ...
    child: Row(
      children: <Widget>[
        CountryCodePicker(
          ...
          boxDecoration: boxDecorationWithRoundedCorners(borderRadius: radius(4), backgroundColor: Colors.white) as BoxDecoration,
...
PreferredSizeWidget faAppbarWidget(BuildContext context, {required String title, List<Widget>? actions, backgroundColor, iconColor, textColor}) {
  return appBarWidget(
    title,
    showBack: true,
...
Widget pinCodeTextFieldWidget(BuildContext context, {StreamController<ErrorAnimationType>? errorController, required TextEditingController controller, required Function(String) onChanged}) {
...
Widget verifyCard({required String image, required String title, required String description, required String iconHeadingText, required String iconHeading}) {
  return Container(
...
Widget createPasswordWidget({required String title, required Color iconColor}) {
  return Row(
    children: [
...
InputDecoration faAddressInputDecoration({required String hint}) {
  return InputDecoration(
    counter: Offstage(),
    hintText: hint,
    hintStyle: secondaryTextStyle(),
...
Widget verifyIdentityCircleWidget({required Color color1, required Color color2, icon}) {
  return Container(
      height: 60,
      width: 60,
      decoration: boxDecorationWithRoundedCorners(
...
Widget sendMoneyUserWidget({required String image, required String name, required String type}) {
  return Row(
    children: [
...
InputDecoration faAmountInputDecoration({required String hint, required String prefixIcon}) {
  return InputDecoration(
    hintText: hint,
    counter: Offstage(),
...
Widget faTitleWidget({required String title, Color? titleColor, required TextStyle subTitleStyle, required String subTitle}) {
  return Row(
...
Widget faActivityIncomeWidget({required String image, required String title1, required String title2}) {
  return Row(
